...
export function AmbienceSection({ images }: { images: string[] }) {
  ...
  if (images.length === 0 || !SITE.instagram) return null;
    ...
    <section className="border-t border-noche-border bg-noche-bg px-6 py-28">
      ...
      >
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-4 font-display text-4xl text-noche-ink">{instagram.handle}</h2>
          <p className="mt-4 text-noche-ink/70">
            Descubre nuestros platos, la terraza y el ambiente de cada día.
          </p>
        </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {images.map((src, index) => (
            ...
            >
              <Image
                src={src}
                alt={`Ambiente de La Osa ${index + 1}`}
                fill
